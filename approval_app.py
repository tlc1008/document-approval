...
# --- 日本語フォントのセットアップ（.ttf形式に変更してエラー回避） ---
@st.cache_resource
def load_font():
    # IPAフォント（明朝）のダウンロードURL
    font_url = "https://github.com/itsc-lab/ipa-fonts/raw/master/ipam.ttf"
    font_path = "ipam.ttf"
    if not os.path.exists(font_path):
        try:
            urllib.request.urlretrieve(font_url, font_path)
        except:
            return 'Helvetica' # 万が一失敗した場合は標準フォント
    
    try:
        pdfmetrics.registerFont(TTFont('IPAMincho', font_path))
        return 'IPAMincho'
    except:
        return 'Helvetica'
...
try:
    ...
    if os.path.exists(file_path):
        ...
        # プレビューが見れない時用の予備リンク
        with open(file_path, "rb") as f:
            st.download_button("⚠️ プレビューが表示されない場合はこちらで確認", f, "check_document.pdf")
    else:
    ...
    st.error(f"システムエラーが発生しました。ログを確認してください。")
    # 詳細なエラーを開発者モードの時だけ出す（任意）
